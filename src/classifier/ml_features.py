...
def create_semantic_similarity_features(df, old_col, new_col, feature_cols, string_mask):
    ...
    if 'label' in old_col:
        ...
        old_texts = [
            f"Entity: {entity}, Entity Type: {entity_type}, Entity Description: {latest_description}, Property: {prop}, Old Value: {old}" 
            for prop, old in zip(subset['property_label'].astype(str), subset[old_col].astype(str))
        ]
        new_texts = [
            f"Entity: {entity}, Entity Type: {entity_type}, Entity Description: {latest_description}, Property: {prop}, New Value: {new}" 
...
def create_time_features(df, feature_cols):
    ...
    def calc_sign_change(row):
        ...
        if dt1.replace('+', '').replace('−', '') == dt2.replace('+', '').replace('−', ''):
            return 1
        else:
            return 0
    ...
    df.loc[valid_mask, 'sign_change'] = df.loc[valid_mask].apply(calc_sign_change, axis=1)
...
def create_globe_coordinate_features(df, feature_cols):
    ...
    for idx in df[coordinate_mask].index:
        try:
            ...
            df.loc[idx, 'latitude_precision_diff_abs'] = abs(get_decimal_places(str(new_val['latitude'])) - get_decimal_places(str(old_val['latitude'])))
            df.loc[idx, 'longitude_precision_diff_abs'] = abs(get_decimal_places(str(new_val['longitude'])) - get_decimal_places(str(old_val['longitude'])))
            
            df.loc[idx, 'latitude_sign_change'] = (float(new_val['latitude']) * float(old_val['latitude']) < 0).astype(int)
            df.loc[idx, 'longitude_sign_change'] = (float(new_val['longitude']) * float(old_val['longitude']) < 0).astype(int)
    ...
    feature_cols.extend([
        ...
        'latitude_precision_diff_abs',
        'longitude_precision_diff_abs',
