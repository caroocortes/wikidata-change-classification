...
SELECT 
    ...
    latitude_new,
    comment,
    -- extract degrees, minutes, seconds from comment and compute decimal
    -- then compare with new value
	(regexp_match(comment, '(\d+)°(\d+)''(\d+\.?\d*)"'))[1]::float as degrees,
	(regexp_match(comment, '(\d+)°(\d+)''(\d+\.?\d*)"'))[2]::float as minutes,
	(regexp_match(comment, '(\d+)°(\d+)''(\d+\.?\d*)"'))[3]::float as seconds,
    abs(
