...
ALTER TABLE <change>
ADD COLUMN IF NOT EXISTS textual_change_predicted BOOLEAN DEFAULT FALSE;

-- REMOVED THIS, don't think I was being fair to consider only those changes stable for 30 days after
...
UPDATE <change> c
SET textual_change_predicted = TRUE
FROM <change_metadata> cm
WHERE 
textual_change_predicted = FALSE AND 
re_formatting_predicted = FALSE AND -- so it doesn't overlap with formatting changes
c.datatype IN ('monolingualtext', 'string', 'url', 'commonsMedia', 'geo-shape', 'tabular-data', 'math', 'musical-notation') AND
NOT (old_value->>0 ~ '[0-9[:punct:]]' AND new_value->>0 ~ '[0-9[:punct:]]') AND
c.action = 'UPDATE' AND 
c.target = 'PROPERTY_VALUE' AND
cm.revision_id = c.revision_id AND
cm.property_id = c.property_id AND
cm.value_id = c.value_id AND
cm.change_target = c.change_target AND
cm.change_metadata = 'CHANGE_MAGNITUDE' AND
cm.value BETWEEN 1 AND 2 AND 
-- days_stable_after > 30 AND -- REMOVED THIS, don't think I was being fair
