...
--- DESCRIPTION OF OLD VALUE
ALTER TABLE gold_standard
ADD COLUMN old_value_description VARCHAR(255) DEFAULT '';
WITH ranked_descriptions AS (
    SELECT 
        r.entity_id, 
        vc.new_value AS entity_description,
        ROW_NUMBER() OVER (PARTITION BY r.entity_id ORDER BY r.timestamp DESC) as rn
    FROM revision r
    JOIN value_change vc ON r.revision_id = vc.revision_id
    WHERE vc.property_id = -2  -- property_id -2 is description
),
latest_descriptions AS (
    SELECT entity_id, entity_description
    FROM ranked_descriptions
    WHERE rn = 1
)
UPDATE gold_standard gs
SET old_value_description = ld.entity_description
FROM latest_descriptions ld
WHERE gs.old_value->>0 = 'Q' || ld.entity_id::text
  AND gs.old_value->>0 ILIKE 'Q%';

--  DESCRIPTION OF NEW VALUE
ALTER TABLE gold_standard
ADD COLUMN new_value_description VARCHAR(255) DEFAULT '';
WITH ranked_descriptions AS (
    SELECT 
        r.entity_id, 
        vc.new_value AS entity_description,
...
UPDATE gold_standard gs
SET new_value_description = ld.entity_description
FROM latest_descriptions ld
WHERE gs.new_value->>0 = 'Q' || ld.entity_id::text
  AND gs.new_value->>0 ILIKE 'Q%';